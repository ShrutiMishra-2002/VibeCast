...
const Podcast = require("../models/category");
const User = require("../models/user")
// add-podcast
router.post("/add-podcasts",authMiddleware, async(req,res)=>{
try{
    const {title,description,category} = req.body;
    const frontImage = req.files["frontImage"][0].path;
    const audioFile = req.files["frontImage"][0].path;
...
if(data && data.podcasts){
    data.podcasts.sort(
        (a,b) => new Data(b.createdAt) - new Data(a.createdAt)
    );
}
