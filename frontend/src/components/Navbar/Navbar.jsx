    ...
    <nav className="px-4 md:px-8 lg:px-12 py-4 relative shadow-md bg-white">
      <div className="flex items-center justify-between">
        ...
        <div className="logo brand-name flex items-center gap-3">
          ...
          <Link to="/" className="text-xl font-bold">Podcaster</Link>
        </div>
        ...
        {/* Show Profile link only if logged in */}
          {isLoggedIn && (
            <Link to="/profile" className="hidden lg:block px-6 py-2 text-lg border rounded-full bg-black font-semibold text-white hover:text-green-100">
              Profile
            </Link>
          )}
        ...
        {!isLoggedIn && (
          <div className="hidden lg:flex space-x-4">
            <Link to="/login" className="px-6 py-2 text-lg border border-green-900 text-green-900 rounded-full">
              Login
            </Link>
            <Link to="/signup" className="px-6 py-2 text-lg bg-green-900 text-white rounded-full">
              Signup
            </Link>
          </div>
        )}
      ...
      {/* Mobile Nav */}
      <div
        className={`lg:hidden fixed top-0 left-0 w-full h-full bg-green-50 z-50 transform ${
